	...
	*/
	...
	function show_thumb($instance) {
		...
		if ( function_exists('the_post_thumbnail') &&
			...
			if( isset($instance["responsive"]) ){
				$ret .= '<a ';
				$ret .= 'class="same-category-post-thumbnail responsive"';
				$ret .= 'href="' . get_the_permalink() . '" title="' . get_the_title() . '">';
				$thumbnail = wp_get_attachment_image_src( get_post_thumbnail_id( get_the_ID() ), 'puntozero_blog_thumb' );
				$ret .= '<img src="' . $thumbnail['0'] . '" class="img-responsive" alt="' . get_the_title() .'" />';
				$ret .= '</a>';
			}
			else {
				$ret .= '<a ';
				$use_css_cropping = isset($instance['use_css_cropping']) ? "same-category-post-css-cropping" : "";
				$ret .= 'class="same-category-post-thumbnail ' . $use_css_cropping . '"';
				$ret .= 'href="' . get_the_permalink() . '" title="' . get_the_title() . '">';
				$ret .= $this->the_post_thumbnail( array($instance['thumb_w'],$instance['thumb_h']));
				$ret .= '</a>';
			}
		}
		return $ret;
    ...
    function itemHTML($instance,$current_post_id) {
		...
		if ( isset( $instance['date'] ) ) {
			$ret .= '<p class="post-date">' . '<span class="post-day">' . get_the_time("j") . '</span> <span class="post-month-year">' . get_the_time("M Y") . '</span></p>';
		}
			...
			?>
				...
				<p>
					<label for="<?php echo $this->get_field_id("responsive"); ?>">
						<input type="checkbox" class="checkbox" id="<?php echo $this->get_field_id("responsive"); ?>" name="<?php echo $this->get_field_name("responsive"); ?>"<?php checked( (bool) $instance["responsive"], true ); ?> />
						<?php _e( 'Responsive' ); ?>
					</label>
				</p>
